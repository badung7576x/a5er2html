...
- **レイアウトの並べ替え** — テーブルカードやノートをドラッグして配置を変更。
  リレーションはリアルタイムで追従します。配置はブラウザ（localStorage）に
  保存され、再読み込み時に復元。**Reset layout** ボタンで元の `.a5er` の
  位置に戻せます。ストレージが使えない環境でも動作します。
...
| `1` | 100% 表示 |
| `/` | 検索ボックスにフォーカス |
| `Esc` | 検索 / 選択をクリア、ドラッグをキャンセル |
| テーブルカード / ノートをドラッグ | 移動 — リレーションが追従、`Esc` で元の位置に戻る |
...
- ビューアは**読み取り専用**です — `.a5er` ファイルの編集・書き戻しは行いません
  （ドラッグした位置はビューア / ブラウザ内のみに保存され、ソースには影響しません）。
